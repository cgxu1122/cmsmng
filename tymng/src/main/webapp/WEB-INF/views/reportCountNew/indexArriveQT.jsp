...
function searchEvt() {
    ...
    $('#dg').datagrid({
        ...
        queryParams: {groupId: 3, startDate: startDate, endDate: endDate, ua: ua, channelIdCondition: channelIdCondition}
    });
}
...
function exportData() {
    ...
    $("body").showLoading();
    $.ajax({
        url: "<%=basePath%>/tymng/reportCountNew/exportArriveData?groupId=3&exportType=3&startDate=" + startDate + "&endDate=" + endDate + "&ua=" + ua + "&channelIdCondition=" + channelIdCondition,
        success: function (result) {
            $("body").hideLoading();
            var result = eval('(' + result + ')');
            if (result.errorMsg) {
